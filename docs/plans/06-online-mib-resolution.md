...
Status: implemented (automated acceptance complete; cross-device/manual source checks remain)
...
- The IANA enterprise registry is persisted for 30 days and reused across OIDs. oid-base requests are serialized with a one-second minimum interval; aggregate lookup results remain cached for seven days.
- Unknown-OID results inspect validated cached-but-unloaded modules in a forked catalog and offer cache-only loading, indexed candidate resolution, and an Observium human-reference link-out.
- Automated proof covers a fresh install making zero resolver HTTP calls before the master switch, cached offline replay, cached-but-unloaded loading, IANA request reuse, oid-base rate limiting, candidate source ordering, and bounded content-validation diagnostics.
- Release evidence still must record the resolver flow on Android and recheck live built-in source URLs at release time; source endpoints can rot independently of the shipped implementation.
