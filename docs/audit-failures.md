...
**Status:** Remediation and exact-candidate revalidation are complete, and the historical v0.6.0 release remains successfully published. Verified master commit `c85ddc651d1056eca0865640b366b9ce5a5f848a` passed all 58 browser scenarios, all 58 versioned matrix checks, the full 1,292-test inventory (1,288 passed, 4 skipped), and the rebuilt Android notification/share harness. AF-003 and AF-009 are resolved; the retained evidence is now tied to pushed `origin/master`.
...
- Local `master` and `origin/master` both point to `c85ddc651d1056eca0865640b366b9ce5a5f848a`. This includes the three post-release product commits (`fa04996`, `875140d`, `be5fe80`), implementation commit `fca1182`, and evidence/report commit `c85ddc6`.
- The dereferenced annotated `v0.6.0` tag remains at released commit `7eb855a4bc037677955b9d113030d9f44cede52f`; it was not moved when `origin/master` advanced to `c85ddc6`. The annotated tag object is `1a238c16184540a07375f8ab65bab7218b3894e5`.
- Verified master contains the intended Command Palette integration, Android native-effects harness, and browser-audit work. Unrelated `docs/plans/post-v1/` files remained outside the audit scope and were temporarily moved only while clean-worktree attestations ran, then restored unchanged.
- A fresh production web bundle from verified master commit `c85ddc651d1056eca0865640b366b9ce5a5f848a` passed all **58/58** browser scenarios in `docs/audits/ui-browser/coverage.json` from a clean worktree and all **58/58** versioned checks in `docs/audits/validation-matrix-v1.json`, with zero failures.
...
Completion status as of 2026-08-13:

No locally solvable audit or publication gate remains. The historical `v0.6.0` tag and release stay unchanged.

AF-003, AF-006, AF-007, and AF-009 are resolved against verified master commit `c85ddc6`; the retained Android harness observed a visible, non-intercepted product notification and a receiver read the exact PNG attachment.
...
| Mobile, tablet, and desktop validation       | Resolved | Exact verified master commit passed browser audit and matrix v1, each 58/58                      |
...
| Keyboard-suitable actions in Command Palette | Resolved | 84 catalog actions and the rendered off-route handoff passed on the exact verified master commit |
...
**Current revalidation status (2026-08-13): Resolved against verified master commit `c85ddc6`.** The candidate defines 84 unique palette-required actions across Browse (5), Tools (18), Traps (8), Packet Console (4), Agents (7), Live MIBs (5), and Settings (37), plus 26 explicitly justified contextual-only interaction classifications. Owner-exact registration rejects missing or unexpected IDs, catalog actions require keyboard-suitable and palette-exposed metadata, and migrated pointer controls dispatch through the registry confirmation gateway. Persistent navigation fallbacks keep all catalog IDs discoverable while their owning route is unmounted. The complete workspace typecheck, ESLint, 1,288 runnable tests, focused action tests, production build, and exact rendered browser matrices pass. The browser scenario selects `tools:start-ports` from Browse, confirms safe navigation while the palette remains open, and observes the mounted Tools action taking ownership.
...
- [x] Android emulator notification permission and delivery are proven against the exact verified master commit.
...
- [x] Android emulator receiver reads the actual PNG attachment from the exact verified master commit.
...
**Current revalidation status (2026-08-13): Resolved against verified master commit `c85ddc651d1056eca0865640b366b9ce5a5f848a`.** The clean-worktree production browser report passes 58/58 scenarios, the versioned matrix passes 58/58 checks, the full suite passes 1,292 tests (1,288 passed and four intentionally skipped), and the signed/rebuilt native harness passes both effects. The browser and Android evidence name the exact verified master commit, contain no stale route failures, and retain portable checksums. The committed CI/release configuration validates those assertions and keys evidence to the tested SHA; the already-published v0.6.0 workflow was not rerun.
...
- [x] The hardened Android harness verifies notification delivery and binary sharing against the exact verified master commit.
- [x] Final browser and native evidence is tied to the exact verified master commit.
...
Every final validation claim identifies the tested Git commit. The prior v0.6.0 evidence remains historical rather than proof of this post-release candidate. The complete affected matrix was rerun against clean verified master commit `c85ddc651d1056eca0865640b366b9ce5a5f848a`; the retained browser and native reports record that SHA, while this follow-up commit changes evidence and documentation only.
...
- `origin/master` now resolves to the verified post-release audit commit `c85ddc651d1056eca0865640b366b9ce5a5f848a`; `refs/tags/v0.6.0^{}` remains at released commit `7eb855a4bc037677955b9d113030d9f44cede52f`, and annotated tag object `refs/tags/v0.6.0` remains `1a238c16184540a07375f8ab65bab7218b3894e5`.
