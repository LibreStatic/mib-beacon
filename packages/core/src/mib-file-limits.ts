import type { MibTextFile } from '@mibbeacon/smi';
...
export function validateMibFileBatch(files: readonly MibTextFile[]): void {
  if (files.length > MAX_MIB_FILE_COUNT) {
    throw new MibBeaconError('CONTENT_VALIDATION_FAILED', 'MIB batch exceeds 1,000 candidates');
  }
  let total = 0;
  for (const file of files) {
    const bytes = new TextEncoder().encode(file.content).byteLength;
    if (bytes > MAX_MIB_FILE_BYTES) {
      throw new MibBeaconError('CONTENT_VALIDATION_FAILED', `${file.name} exceeds the 5 MiB file limit`);
    }
    total += bytes;
    if (total > MAX_MIB_BATCH_BYTES) {
      throw new MibBeaconError('CONTENT_VALIDATION_FAILED', 'MIB batch exceeds the 50 MiB total limit');
    }
  }
}
