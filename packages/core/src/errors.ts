 ...
 */
export type MibBeaconErrorCode =
  // transport
  | 'TIMEOUT'
  | 'HOST_UNREACHABLE'
  | 'PORT_BIND_DENIED'
...
export interface MibBeaconErrorOptions {
  hint?: string;
  cause?: unknown;
  details?: Record<string, unknown>;
}

export class MibBeaconError extends Error {
  readonly code: MibBeaconErrorCode;
  readonly hint?: string;
  readonly details?: Record<string, unknown>;

  constructor(code: MibBeaconErrorCode, message: string, opts: MibBeaconErrorOptions = {}) {
    super(message, opts.cause !== undefined ? { cause: opts.cause } : undefined);
    this.name = 'MibBeaconError';
    this.code = code;
    this.hint = opts.hint;
...
/**
 * Map a raw node-net-snmp error (or any thrown value) to an MibBeaconError.
 * node-net-snmp error class names are stable identifiers we key on.
 */
export function mapSnmpError(err: unknown): MibBeaconError {
  if (err instanceof MibBeaconError) return err;
  const e = err as { name?: string; message?: string } | null;
  const name = e?.name ?? '';
  const msg = e?.message ?? String(err);

  if (name === 'RequestTimedOutError' || /timed out|timeout/i.test(msg)) {
    return new MibBeaconError('TIMEOUT', 'Request timed out', {
  ...
  if (/EHOSTUNREACH|ENETUNREACH|not reachable/i.test(msg)) {
    return new MibBeaconError('HOST_UNREACHABLE', 'Host unreachable', { cause: err });
  }
  if (/EACCES|permission denied/i.test(msg)) {
    return new MibBeaconError('PORT_BIND_DENIED', 'Permission denied binding the port', {
  ...
  if (/usmStatsDecryptionErrors|decryption error/i.test(msg)) {
    return new MibBeaconError('V3_DECRYPT_FAILED', 'SNMPv3 decryption failed', {
  ...
  if (/usmStatsUnknownEngineID|unknown engine/i.test(msg)) {
    return new MibBeaconError('V3_UNKNOWN_ENGINE_ID', 'SNMPv3 engine ID discovery pending', {
      cause: err,
    });
  }
  ...
  return new MibBeaconError('REQ_FAILED', msg || 'SNMP request failed', { cause: err });
}
