...
export function PacketConsole({ host }: { host?: AppHostAdapter }) {
  ...
  useEffect(
    () => setSize((value) => Math.max(layout.minSize, Math.min(layout.maxSize, value))),
    [layout.maxSize, layout.minSize],
  );
  ...
  const exportCapture = async () => {
    ...
    try {
      ...
      id = descriptor.id;
      if (!ownsEngine() || exportAttempt.current !== attempt) return;
  ...
  return (
    <View nativeID="packet-console" style={shellStyle} pointerEvents="box-none">
      <View
        {...pan.panHandlers}
        style={[styles.dragZone, layout.edge === 'top' ? styles.dragTop : styles.dragBottom]}
      >
        <Pressable
        ...
        <View style={styles.consoleBody}>
          <View style={[styles.toolbar, mode === 'compact' ? styles.toolbarMobile : null]}>
            <View style={styles.consoleTitleWrap}>
              <Text numberOfLines={1} style={styles.consolePrompt}>
                mibbeacon://wire
              </Text>
              {mode !== 'compact' ? (
                <Text style={styles.consoleMeta}>UDP PAYLOAD INSPECTOR · LIVE</Text>
              ) : null}
            </View>
            <View
              style={[
                styles.toolbarActions,
                mode === 'compact' ? styles.toolbarActionsMobile : null,
              ]}
            >
              <Button
          ...
          <View style={[styles.filters, mode === 'compact' ? styles.filtersMobile : null]}>
            ...
            <View style={styles.filterGroup}>
              {(['all', 'valid', 'invalid'] as const).map((value) => (
                <Chip
                  key={`validity-${value}`}
                  label={value === 'all' ? 'ANY' : value === 'valid' ? 'OK' : 'ERR'}
                  active={validity === value}
                  onPress={() => setValidity(value)}
                />
              ))}
            </View>
          ...
          {message ? (
            <Text
              style={{
                color: message.startsWith('Exported') ? CONSOLE_OK : CONSOLE_ERROR,
                fontSize: 10,
                ...
                <>
                  <View style={styles.hexHead}>
                    <Text style={styles.hexTitle}>
                      {selected.direction.toUpperCase()} · {selected.operation.toUpperCase()} ·{' '}
                      {selected.byteLength} BYTES
                    </Text>
                    <Text
                      style={[
                        styles.hexStatus,
                  ...
                  <ScrollView
                    style={styles.hexViewport}
                    contentContainerStyle={styles.hexViewportContent}
                    nestedScrollEnabled
                  >
                    <ScrollView
                      horizontal
          ...
          <View style={[styles.footer, mode === 'compact' ? styles.footerMobile : null]}>
            <Text style={styles.footerText}>
              Raw SNMP may expose community strings or unencrypted values. History:{' '}
              {status?.persistence ?? 'loading'} · {status?.retentionMiB ?? 32} MiB cap.
            </Text>
          </View>
        </View>
      ) : null}
...
function PacketRow({
  ...
  return (
    <Pressable
      onPress={onPress}
      style={[styles.packetRow, selected ? styles.packetRowSelected : null]}
    >
      <Text style={[styles.direction, { color }]}>
        {packet.direction === 'tx' ? '→ TX' : '← RX'}
      </Text>
      <View style={{ flex: 1 }}>
        <Text style={styles.packetHeadline} numberOfLines={1}>
          {packet.operation.toUpperCase()} · {packet.remoteAddress ?? 'unknown'}:
          {packet.remotePort ?? '—'}
        </Text>
        <Text style={styles.packetMeta}>
          {new Date(packet.timestamp).toLocaleTimeString()} · {packet.transport} ·{' '}
          {packet.byteLength} B
        </Text>
      </View>
    </Pressable>
...
const styles = StyleSheet.create({
  ...
  mobileShell: {
    ...
    backgroundColor: CONSOLE_BG,
    borderBottomWidth: 1,
    borderBottomColor: CONSOLE_LINE,
    overflow: 'hidden',
    shadowColor: consolePalette.shadow,
    shadowOpacity: 0.45,
    shadowRadius: 18,
  ...
  warning: {
    ...
    paddingHorizontal: 10,
    paddingVertical: 5,
    backgroundColor: consolePalette.warnBg,
  },
